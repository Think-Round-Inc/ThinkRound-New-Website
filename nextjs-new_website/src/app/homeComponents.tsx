...
export const homeComponents: PortableTextComponents = {
    types: {
        ...
        videoBlock: ({ value }) => {
            ...
            const originalHeight =
                videoFile?.asset?.metadata?.dimensions?.height;
                        ...
                        >
                            ...
                            >
                                <source
                                    ...
                                    type={
                                        videoFile.asset.mimeType || "video/mp4"
                                    }
                                />
                ...
                <div className='my-10 flex flex-wrap items-center justify-center gap-4'>
                    {buttons.map(
                        (button: Record<string, unknown>, index: number) => (
                            <a
                                key={
                                    (button?._key as string) ||
                                    `${button?.href}-${button?.label}-${index}`
                                }
    ...
    block: {
        normal: ({ children }) => (
            <p className='whitespace-pre-wrap text-lg font-light mb-6 leading-relaxed'>
                {children}
            </p>
        ),
